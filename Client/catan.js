...
function roadBuliding() {
	...
	onInput = function(x, y) {
		...
		if(pos != null) {
			...
			showMessage("You may now place a road. Click where you would like to place your road.");
			drawBoard(0, true);
			onInput = function(x, y, r=pos.hex.diagonal + " " + pos.hex.column + " " + pos.link) {
				var pos = calculateLinkClicked(x, y);
				if(pos != null) {
...
function isSameRoad(first, second) {
	...
	first.offset = {x: xOffsetGlobal + first.col * HEX_WIDTH * 3 / 4, y: yOffsetGlobal + first.diag * HEX_HEIGHT - first.col * HEX_HEIGHT / 2};
	second.offset = {x: xOffsetGlobal + second.col * HEX_WIDTH * 3 / 4, y: yOffsetGlobal + second.diag * HEX_HEIGHT - second.col * HEX_HEIGHT / 2};
